...
# with a defined [step]. With a range with one parameter, the [start] is set to
# 0 and [step] is set to 1. A [step] of 0 will raise a ValueError.
...
# range(...) implements __getitem__() which allows to use braces to get a value
# from an index. Slices can also be used.
...
# different ways. For example, we can loop through the first four elements:
...
# will print integers 9 to 0 (in that order):
